...
echo "   (59-aectl-oauth-apps.sh — registers console / api / workload-publisher / 3x bff→service clients)"
...
if [ "$PIN_RC" = "2" ]; then
    ...
    echo "   stay ErrImageNeverPull (pullPolicy: Never, no registry). Re-run setup-aectl.sh."
    exit 1
fi
...
metadata:
  name: aectl-api-client-binding
spec:
  effect: allow
  entitlement:
    claim: sub
    value: aectl-api-client
  roleMappings:
  - roleRef:
...
# Anthropic secret write) runs in aectl-api's detached goroutine as
# sub=aectl-api-client (admin *), so `component:create` is the ONLY action this
...
[ -z "$GITHUB_APP_SLUG_VAL" ] && GITHUB_APP_SLUG_VAL="aectl-platform"
...
# every time setup-aectl.sh regenerates the .env from scratch.
...
cat > "$ENV_FILE" <<EOF
# Auto-generated by setup-aectl.sh — $(date -u +"%Y-%m-%dT%H:%M:%SZ")
#
# Re-running setup-aectl.sh preserves secrets, the smee.io channel, and any
# values you've hand-edited (ANTHROPIC_API_KEY, GitHub App credentials).

# ── OpenChoreo Platform API ────────────────────────────────────────────────
# aectl-api (in compose) reaches OC via the k3d Docker network. The Host
...
VITE_THUNDER_CLIENT_ID=aectl-console-client
...
# rotate by clearing the values here and re-running setup-aectl.sh.
...
GIT_COMMITTER_EMAIL=bot@aectl.dev
...
# `services/aectl-api/internal/platform/tenant/namespace.go::OrgBaseNamespace`.
echo ""
...
SEEDER_CLIENT_ID="${SEEDER_CLIENT_ID:-aectl-local-dev-seeder}"
SEEDER_CLIENT_SECRET="${SEEDER_CLIENT_SECRET:-aectl-local-dev-seeder-secret}"
