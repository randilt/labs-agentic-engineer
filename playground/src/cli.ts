...
function printUsage(): void {
  output.write(
    [
      ...
      "",
      "Env:",
      "  ANTHROPIC_API_KEY         the engineering agent's key (env or deployments/.env)",
      "  AEP_CODING_ANTHROPIC_KEY  bill CODING runs separately — an API key, or a",
      "                            `claude setup-token` token to bill your Claude",
      "                            subscription. Used by docker mode, and by --host",
      "                            --api-key; a bare --host still uses claude login.",
      "",
