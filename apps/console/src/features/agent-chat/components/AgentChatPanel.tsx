...
// The command for a one-shot generation signal.
//
// Design is derived from the current requirements. Requirements go through
// `/start`, sent VERBATIM: the SERVER classifies it and attaches the idea
// captured at project creation, read from the project's own descriptor
// (specs/.agentic-engineer.toml). The console composes nothing and stores
// nothing — which is what makes the idea survive a different browser, device,
// or teammate, where the old localStorage copy did not. The start skill owns
// the interview.
function instructionFor(signal: "requirements" | "design" | "onboard"): string {
  if (signal === "design") return DESIGN_COMMAND;
  if (signal === "onboard") return ONBOARD_COMMAND;
...
export function AgentChatPanel({
  ...
  autoGenerate?: "requirements" | "design" | "onboard";
  ...
  useEffect(() => {
    ...
    if (!engagedRef.current) {
      ...
      sendRef.current(instructionFor(autoGenerate));
    }
    onAutoGenerated?.();
